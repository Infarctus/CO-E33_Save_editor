...
const RessourcesPanel: React.FC<GeneralPanelProps> = ({
    jsonMapping,
    triggerSaveNeeded,
}) => {
    ...
    const MiscItemsDict = useMemo(() => {
        ...
        return Object.fromEntries(
            MiscItemsInv.map(([name]) => [name, itemMap[name] ?? 0])
        )
    }, [jsonMapping])
        ...
        <div id="RessourcesPanel" className="tab-panel overflow-auto">
            ...
            <table>
                <tbody>
                    {Array.from({
                        length: Math.max(
                            Object.keys(MiscItemsDict).length,
                            Object.keys(TintsLvl).length
                        ),
                    }).map((_, idx) => {
                        ...
                        if (miscItemEntry) {
                            ...
                            const friendlyName =
                                MiscItemsInv.find(([n]) => n === name)?.[1] ||
                                name
                            miscCell = renderNumberInput(
                                value,
                                friendlyName,
                                0,
                                99,
            ...
            {Object.entries(upgradeWeaponMats).map(([name, level]) => {
                ...
                const friendlyName =
                    upgradeWeaponMatsdef.find(([baseName]) =>
