...
const MusicDisksPanel: React.FC<GeneralPanelProps> = ({
    ...
    const [musicDisks, setMusicDisks] =
        useState<MusicDisckInfo[]>(initialMusicDisks)
    const [searchQuery, setSearchQuery] = useState<string>('')
    const [sortField, setSortField] = useState<SortField>(null)
    const [sortDirection, setSortDirection] = useState<SortDirection>('asc')

    const handleMusicDiskChange = (
        musicDisksname: string,
        newFound: boolean
    ) => {
        let thisMusicDiskWas: MusicDisckInfo | undefined
        thisMusicDiskWas = musicDisks.find(
            (musicDisk) => musicDisk.name === musicDisksname
        )
        if (!thisMusicDiskWas) {
            ...
            if (thisMusicDiskWas.found && newFound === false) {
                jsonMapping.root.properties.InventoryItems_0.Map =
                    jsonMapping.root.properties.InventoryItems_0.Map.filter(
                        (inventoryEl) =>
                            inventoryEl.key.Name === musicDisksname
                                ? false
                                : true
                    )
