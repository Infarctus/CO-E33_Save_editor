...
const CharactersPanel: FC<CharactersPanelProps> = ({
    ...
    if (
        ...
        return (
            <div id="CharactersPanel" className="tab-panel">
                <h2>Characters</h2>
                <p style={{ color: 'red' }}>
                    The file you opened (if any) doesn't look like a CO:E33 save
                    file
                </p>
            </div>
        )
    ...
    return (
        <div id="CharactersPanel" className="tab-panel">
            ...
            >
                {jsonMapping.root.properties.CharactersCollection_0.Map.map(
                    (character, index) => (
                        <CharacterSection
                            key={index}
                            character={character}
                            ...
                            currentSkins={getUnlockedSkinsFor(
                                character.key.Name,
                                jsonMapping.root.properties.InventoryItems_0.Map.map(
                                    (el) => el.key.Name
                                )
                            )}
                            allowedSkins={getPossibleSkinsFor(
                                character.key.Name
                            )}
                            allowedCustomizationsFace={getPossibleFacesFor(
                                character.key.Name
                            )}
            ...
            <div style={{ marginTop: '1rem' }}>
                ...
                <PropertyEditor
                    ...
                    onChange={(newValue) => {
                        ...
                        jsonMapping.root.properties.CharactersCollection_0.Map[
                            characterIndex
                        ].value.Struct.Struct.CurrentLevel_49_97AB711D48E18088A93C8DADFD96F854_0.Int =
                            clamp(Number(newValue), 1, 99)
                        logAndInfo(
                            `Character ${characterName} CurrentLevel updated to ${newValue}`
                        )
                    }}
                />
                <PropertyEditor
                    ...
                    onChange={(newValue) => {
                        ...
                        jsonMapping.root.properties.CharactersCollection_0.Map[
                            ...
                            Number(newValue)
                        logAndInfo(
                            `Character ${characterName} LuminaFromConsu updated to ${newValue}`
                        )
                    }}
                />
                ...
                >
                    ...
                    {Object.entries(
                        ...
                        const currpointlabel = Number.parseInt(
                            points.key.Byte.Label.slice(-1)
                        )
                        return (
                            <PropertyEditor
                                key={index}
                                labelText={getECharacterAttributeEnumValue(
                                    currpointlabel
                                )}
                                value={points.value}
                                onChange={(newValue) => {
                                    ...
                                    logAndInfo(
                                        `Character ${characterName} Attribute ${index} updated to ${newValue}`
                                    )
                                }}
                            />
                        )
        ...
        */}
                ...
                <CharacCustoEditor
                    ...
                    onUpdateSkin={(newList) => {
                        triggerSaveNeeded()
                        const allowedFacesRawNames =
                            allowedCustomizationsFace.map((el) => el[0])
                        ...
                        trace(
                            `Character ${characterName} faces updated to ${newList.join('+ ')}`
                        )
                    }}
                />
                ...
                <CharacCustoEditor
                    ...
                    onUpdateSkin={(newList) => {
                        triggerSaveNeeded()
                        const allowedSkinsRawNames = allowedSkins.map(
                            (el) => el[0]
                        )
                        ...
                        trace(
                            `Character ${characterName} bodies updated to ${newList.join('+ ')}`
                        )
                    }}
                />
                {/* 
