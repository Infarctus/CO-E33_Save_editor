...
const WeaponsPanel: FC<WeaponsPanelProps> = ({
    ...
    const allWeaponsMapping: [string, { [weaponKey: string]: string }][] =
        useMemo(() => {
        ...
        () =>
            Object.fromEntries(
                allWeaponsMapping.map(([owner, weapons]) => {
                    const weaponInfoTypes: WeaponInfoType[] = Object.keys(
                        weapons
                    ).map((name) => {
    ...
    const handleWeaponCheckUpdate = (
        ...
        var thisWeaponWas: WeaponInfoType = weapons[owner].find(
            (e) => e.name === weaponName
        )!
        ...
        logAndInfo(
            'Weapon update:' + weaponName + ' ' + newFound + ' ' + newLevel
        )
    }
    ...
    const displayedWeapons = useMemo(() => {
        let filtered = Object.entries(weapons)
            ...
            .map(([keyOwner, weaponsList]) => {
                const filteredWeaponsList = (
                    weaponsList as WeaponInfoType[]
                ).filter((weapon) => {
                    return weapon.friendlyName
                        .toLowerCase()
                        .includes(searchQuery.toLowerCase())
