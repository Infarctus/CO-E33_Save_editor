...
import {handleSaveFileAndExtractToJson, SaveProcessResult} from "./SaveHandler.ts";
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  SaveFileSelectEl?.addEventListener("click", async (e) => {
    ...
    handleSaveFileAndExtractToJson().then((saveProcessResult) => {
      if (!saveProcessResult.success) InfoBannerEl!.innerText = saveProcessResult.message;
      else InfoBannerEl!.innerText = `OK`;
    }).catch((reason) => InfoBannerEl!.innerText = `Promise rejected. ${reason}`)
  });
});
