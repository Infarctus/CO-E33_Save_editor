...
export interface RootProperties {
  ...
  Gold_0: IntTag;
  WeaponProgressions_0: WeaponProgressions_0; // TODO
...
export function setValueOfTag(
  ...
  if ("Double" in tag) {
    ...
    (tag as IntTag).Int =
      typeof value === "number" ? value : Number.parseInt(value.toString());
  } else if ("Bool" in tag) {
    (tag as BoolTag).Bool =
    ...
    (tag as StringsArrayTag).Array.Base.Name = Array.isArray(value)
      ? value
      : [value.toString()];
  } else {
