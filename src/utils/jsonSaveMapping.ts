 ...
 */
export async function getMappingJsonFromFile(
    jsonPath: string
): Promise<BeginMapping | null> {
    try {
...
export async function saveMappingJsonToDisk(
    ...
    if (!!jsonMapping?.root?.properties?.SaveDateTime_0?.Struct?.DateTime) {
        ...
        const ticks =
            621355968000000000 +
            (now.getTime() - now.getTimezoneOffset() * 60000) * 10000
        jsonMapping.root.properties.SaveDateTime_0.Struct.DateTime = ticks
        // trace("Set SaveDateTime_0 to "+ticks)
    } else {
        error(
            "Could not bump date of the save because the structure doesn't exist"
        )
    }
    try {
