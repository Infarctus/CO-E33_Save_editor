...
export async function handleJsonAndConvertToSaveFile(
    ...
    if (!(await exists(jsonPath))) {
        return {
            success: false,
            message:
                'Temporary JSON file path is invalid or file does not exist.',
        }
    }
    ...
    const intermediateSavPath = await join(
        saveHandlingBasePath,
        `CONVERSION_TEST_${fileName}`
    )
    ...
    try {
        ...
        const verifyCommand = Command.sidecar(
            UESAVE_EXE_PATH,
            uesaveArgsVerifyConversion
        )
        const {
            stdout: verifyStdout,
            stderr: verifyStderr,
